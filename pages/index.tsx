...
const allAppointments: AppointmentModel[] = json.data.allNotes.edges.map((appt: any) => {
	return {
		...
		supervisorName: getFullName(
			appt.supervisor?.account?.firstName,
			appt.supervisor?.account?.lastName
		),
...
const Home: NextPage = () => {
	const [filteredAppointments, setFilteredAppointments] =
		useState<AppointmentModel[]>(allAppointments)
	...
	const columnStatuses = [
		...
		AppointmentStatuses.NEED_REVIEW,
	...
	return (
		<div className={styles.container}>
			<Head>
				<title>Appointments - MedTruly</title>
			</Head>
			<div className={styles.sideNav}>
				...
				<div className={styles.navItemContainer}>
					<div className={styles.filterItemLabel}>period</div>
					<Select
						value={periodFilter}
						style={{ width: '100%' }}
				...
				<div className={styles.navItemContainer}>
					<div className={styles.filterItemLabel}>type of appointment</div>
					<Select
						value={typeFilter}
						style={{ width: '100%' }}
						options={typeOptions}
			...
			<div className={styles.columns}>
				{columnStatuses.map(status => {
					...
					return (
						<Column key={status} title={AppointmentStatusToTitles[status]}>
							{appointments.map(appt => {
								return <AppointmentItem key={appt.id} {...appt} />
							})}
						</Column>
					)
				})}
